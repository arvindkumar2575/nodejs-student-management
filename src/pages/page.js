...
p.htmlMetaData = (d) => {
  let styles = "";
  if (!helper.isEmpty(d.stylesheets)) {
...
p.index = (d) => {
  return `${p.doctype}${p.htmlStart}${p.htmlHeadStart}${p.htmlMetaData(
    d.meta
  )}${p.htmlHeadEnd}${p.htmlBodyStart}${p.htmlBody(d.body)}${p.htmlBodyEnd}${
    p.htmlEnd
  }`;
};
